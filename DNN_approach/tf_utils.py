...
def fc(x, n_output, scope="fc", activation_fn=None, initializer=None):
  ...
  with tf.compat.v1.variable_scope(scope):
    if initializer is None:
      ...
      W = tf.Variable(tf.random.normal([int(x.get_shape()[1]), n_output]))
      b = tf.Variable(tf.random.normal([n_output]))
    else:
      W = tf.compat.v1.get_variable("W", shape=[int(x.get_shape()[1]), n_output], initializer=initializer)
      b = tf.compat.v1.get_variable("b", shape=[n_output],
